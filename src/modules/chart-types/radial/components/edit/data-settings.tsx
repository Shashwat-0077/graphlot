...
import { Donut, Database } from "lucide-react";
...
export function RadialDataSettings({
    chartId,
    userId,
}: {
    chartId: string;
    ...
    const xAxis = useRadialChartStore((state) => state.xAxisField);
    const yAxis = useRadialChartStore((state) => state.yAxisField);
    const sortX = useRadialChartStore((state) => state.xAxisSortOrder);
    const omitZeroValuesEnabled = useRadialChartStore(
        (state) => state.omitZeroValuesEnabled
    );
    const sortY = useRadialChartStore((state) => state.yAxisSortOrder);

    const setDataConfig = useRadialChartStore((state) => state.setRadialConfig);
            ...
            <div className="grid grid-cols-1 gap-4 @[600px]:grid-cols-2">
                ...
                <div className="min-w-0 space-y-4">
                    <div className="flex items-center gap-2">
                        <Donut className="h-4 w-4 flex-shrink-0" />
