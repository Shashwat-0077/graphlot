...
import { FullAreaChartUpdate } from "@/modules/Area/schema";
import {
    ChartBoxModel,
    ChartColors,
    ChartMetadata,
...
export async function updateAreaChartWithRetry({
    chartId,
    data,
    maxRetries = 3,
    retryDelay = 200,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const result = await updateAreaChart({ chartId, data });
            if (result.ok) {
                return result;
            }
