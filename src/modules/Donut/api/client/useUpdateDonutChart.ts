...
type RequestType = InferRequestType<
    (typeof client.api)["donut-chart"][":chart_id"]["$put"]
>;
...
export const useUpdateDonutChart = ({
    ...
    return useMutation<ResponseType, Error, RequestType>({
        mutationFn: async ({ param, json }) => {
            const response = await client.api["donut-chart"][":chart_id"].$put({
                param: param,
                json: json,
            });
        ...
        onSuccess: (data, variables) => {
            ...
            queryClient.invalidateQueries({
                queryKey: ["donut-chart", variables.param.chart_id],
            });

            // Optionally, directly update the cache without waiting for a refetch
            queryClient.setQueryData(
                ["donut-chart", variables.param.chart_id],
                data
            );
        },
    });
