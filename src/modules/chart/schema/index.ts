...
import {
    ...
    defaultChartColors,
} from '@/modules/chart/default-chart-config';
...
export const ChartMetadata = sqliteTable(
    CHART_METADATA_TABLE_NAME,
    {
        ...
        databaseProvider: text('database_provider').notNull().$type<DatabaseType>(),
    ...
    (table) => [unique('uq_charts_collection_name').on(table.collectionId, table.name)],
);
...
export const ChartVisual = sqliteTable(
    CHART_VISUAL_TABLE_NAME,
    {
        ...
        gridStyle: text('grid_type').notNull().$type<GridStyle>().default(defaultChartVisualSettings.gridStyle),
        gridWidth: real('grid_width').notNull().default(defaultChartVisualSettings.gridWidth),
...
export const ChartTypography = sqliteTable(
    CHART_TYPOGRAPHY_TABLE_NAME,
    {
        chartId: text('chart_id')
            .primaryKey()
            .references(() => ChartMetadata.chartId, { onDelete: 'cascade' }),
        label: text('label').notNull().default(defaultChartTypographySettings.label),
        labelEnabled: integer('label_enabled', { mode: 'boolean' })
            .notNull()
            .default(defaultChartTypographySettings.labelEnabled),
        labelSize: real('label_size').notNull().default(defaultChartTypographySettings.labelSize),
...
export const ChartBoxModel = sqliteTable(
    CHART_BOX_MODEL_TABLE_NAME,
    {
        chartId: text('chart_id')
            .primaryKey()
            .references(() => ChartMetadata.chartId, { onDelete: 'cascade' }),
        marginTop: real('margin_top').notNull().default(defaultChartBoxModel.marginTop),
        marginBottom: real('margin_bottom').notNull().default(defaultChartBoxModel.marginBottom),
        marginLeft: real('margin_left').notNull().default(defaultChartBoxModel.marginLeft),
        marginRight: real('margin_right').notNull().default(defaultChartBoxModel.marginRight),
        borderWidth: real('border_width').notNull().default(defaultChartBoxModel.borderWidth),
    },
    // (table) => [
    //     check(
...
export const ChartColors = sqliteTable(CHART_COLOR_TABLE_NAME, {
    ...
    gridColor: text('grid_color', { mode: 'json' }).notNull().$type<RGBAColor>().default(defaultChartColors.gridColor),
