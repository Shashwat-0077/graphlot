...
import { defaultHeatmapConfig } from '@/modules/heatmap/default-heatmap-config';
...
export const HeatmapCharts = sqliteTable(HEATMAP_CHARTS_TABLE_NAME, {
    ...
    dateField: text('date_field').notNull().default(defaultHeatmapConfig.dateField).$type<string>(),
    valueField: text('value_field').notNull().default(defaultHeatmapConfig.valueField).$type<string>(),
    ...
    labelEnabled: integer('legend_enabled', { mode: 'boolean' }).notNull().default(defaultHeatmapConfig.labelEnabled),

    legendEnabled: integer('legend_enabled', { mode: 'boolean' }).notNull().default(defaultHeatmapConfig.legendEnabled),

    borderEnabled: integer('borderEnabled', { mode: 'boolean' }).notNull().default(defaultHeatmapConfig.borderEnabled),
    ...
    accent: text('accent', { mode: 'json' }).notNull().default(defaultHeatmapConfig.accent).$type<RGBAColor>(),
});
