...
type RequestType = InferRequestType<
    (typeof client.api)["bar-chart"][":chart_id"]["$put"]
>;
...
export const useUpdateBarChart = ({
    ...
    return useMutation<ResponseType, Error, RequestType>({
        mutationFn: async ({ param, json }) => {
            const response = await client.api["bar-chart"][":chart_id"].$put({
                param: param,
                json: json,
            });
        ...
        onSuccess: (data, variables) => {
            ...
            queryClient.invalidateQueries({
                queryKey: ["bar-chart", variables.param.chart_id],
            });

            // Optionally, directly update the cache without waiting for a refetch
            queryClient.setQueryData(
                ["bar-chart", variables.param.chart_id],
                data
            );
        },
    });
